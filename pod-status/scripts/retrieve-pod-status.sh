...
for pod in "${pod_array[@]}"; do
    ...
    for ((i=0; i<count; i++)); do
        pod_status=$(kubectl get pod $cleaned_output -n "$APK_RELEASE_NAMESPACE" -o=jsonpath="{.spec.containers[$i]}")
        pod_name=$(echo $pod_status | jq '.name' | sed 's/"//g')
        cpu=$(convert_cpu_to_milli_cores $(echo $pod_status | jq '.resources.limits.cpu' | sed 's/"//g'))
        memory=$(convert_memory_to_bytes $(echo $pod_status | jq '.resources.limits.memory' | sed 's/"//g'))
...
filtered_pod_status=${result%,}
echo $filtered_pod_status
data="{$filtered_pod_status}"
timestamp=$(date +%Y%m%d%H%M%S)
kubectl patch configmap $POD_STATUS_CONFIGMAP --patch "{\"data\": {\"$timestamp\": \"$data\"}}" -n $APK_RELEASE_NAMESPACE
